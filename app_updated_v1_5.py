...
.certificate-title {font-size: 1.55rem; font-weight: 800; color: #0F172A; margin-bottom: 0.25rem;}
.certificate-name {font-size: 1.35rem; font-weight: 800; color: #0F766E; margin: 0.55rem 0;}
.certificate-small {font-size: 0.90rem; color: #475569;}
...
def generate_certificate_png(name: str, score: int, result_label: str) -> bytes:
    ...
    draw.rounded_rectangle((36, 36, width - 36, height - 36), radius=34, fill="#FFFFFF", outline="#0F766E", width=5)
    draw.rounded_rectangle((62, 62, width - 62, height - 62), radius=24, outline="#BAE6FD", width=2)
    draw.rectangle((36, 36, width - 36, 170), fill="#0F766E")
    if LOGO_PATH.exists():
        try:
            logo = Image.open(LOGO_PATH).convert("RGBA")
            logo.thumbnail((150, 82))
            logo_x = int((width - logo.width) / 2)
            img.paste(logo, (logo_x, 70), logo)
        except Exception:
            pass
    y = 205
    y = _draw_centered(draw, "Certificate of Completion", y, _load_font(46, True), "#0F172A", width)
    y = _draw_centered(draw, "EnergyWise Homes Australia", y + 8, _load_font(28, True), "#0F766E", width)
    y = _draw_centered(draw, "This recognises that", y + 24, _load_font(25), "#475569", width)
    name_lines = textwrap.wrap(name, width=28) or [name]
    for line in name_lines[:2]:
        y = _draw_centered(draw, line, y + 4, _load_font(42, True), "#0F172A", width)
    body = "completed the home-energy check-up and built a practical action plan for identifying common household energy-wasting features."
    for line in textwrap.wrap(body, width=74):
        y = _draw_centered(draw, line, y + 4, _load_font(23), "#334155", width)
    y += 18
    y = _draw_centered(draw, f"Result: {result_label} | Score: {score}/100", y, _load_font(26, True), "#0F766E", width)
    y = _draw_centered(draw, f"Date: {today}", y + 5, _load_font(22), "#475569", width)
    footer = "Tech Innovation Experts | Providing technology-driven services across Oceania | tinx@gmail.com"
    _draw_centered(draw, footer, height - 100, _load_font(20), "#475569", width)
    _draw_centered(draw, "Recognition certificate only. Not a certified energy assessment or NCC/NatHERS rating.", height - 68, _load_font(17), "#64748B", width)
    buffer = BytesIO()
...
def plan_screen() -> None:
    ...
    st.markdown("### Recognition certificate")
    st.markdown(
        f"""
        <div class='certificate-card'>
            <div class='certificate-title'>Certificate of Completion</div>
            <div class='certificate-small'>EnergyWise Homes Australia recognises</div>
            <div class='certificate-name'>{participant_name}</div>
            <div class='certificate-small'>for completing the home-energy check-up and building a practical energy action plan.</div>
            <div class='certificate-small'><strong>Result:</strong> {result_text} | <strong>Score:</strong> {st.session_state.score}/100 | <strong>Date:</strong> {completion_date}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )

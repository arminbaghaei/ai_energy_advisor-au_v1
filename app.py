...
# ---------- Commercial engagement layer: challenge, visuals, and subscription logic ----------
...
def subscription_fee_monthly() -> float:
    return float(st.session_state.get("subscription_fee", 19.0))


def annual_subscription_cost() -> float:
    return subscription_fee_monthly() * 12


def roi_summary(low_saving: float, high_saving: float, monthly_fee: float | None = None) -> dict:
    """Compare indicative savings with a possible subscription fee.
    ...
    if monthly_fee is None:
        monthly_fee = subscription_fee_monthly()
...
def render_payoff_strip(low_saving: float, high_saving: float, context: str = "estimated saving opportunity") -> None:
    roi = roi_summary(low_saving, high_saving)
    st.markdown(
        f"""
        <div class='payoff-strip'>
            <div><strong>Subscription value test</strong><br><span>At A${roi['monthly_fee']:.0f}/month, annual access costs {_currency(roi['annual_fee'])}.</span></div>
            <div><strong>{_currency(low_saving)}–{_currency(high_saving)}</strong><br><span>{context}</span></div>
            <div><strong>{roi['multiple_low']:.1f}×–{roi['multiple_high']:.1f}×</strong><br><span>gross saving-to-fee ratio</span></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_recommendation_visual(visual_key: str, height: int = 190) -> None:
    ...
    try:
        ...
        st.caption(f"{data['icon']} {data['caption']}")
    except Exception:
        st.markdown(
            f"""
...
def commercial_teaser_panel(title: str = "Premium advisor mode") -> None:
    snap = st.session_state.get("money_snapshot", {})
    low = float(snap.get("waste_low", 0))
    high = float(snap.get("waste_high", 0))
    roi = roi_summary(low, high)
    st.markdown(
        f"""
        <div class='premium-card'>
            <span class='premium-badge'>Coming next</span>
            <h4>{title}</h4>
            <p>Track actions for 30 days, unlock the next upgrade, and compare every recommendation against your subscription cost.</p>
            <p><strong>Value frame:</strong> A${roi['monthly_fee']:.0f}/month = {_currency(roi['annual_fee'])}/year. Your current estimated opportunity is {_currency(low)}–{_currency(high)}/year.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
.photo-frame img {border-radius:18px; border:1px solid #E2E8F0; box-shadow:0 1px 6px rgba(15,23,42,.08);}
.visual-fallback {
...
.hotspot-money {
    padding:.75rem; border-radius:14px; background:#ECFDF5; border:1px solid #A7F3D0; color:#064E3B; margin:.55rem 0;
}
@media (max-width: 760px) {
...
def money_screen() -> None:
    ...
    episode_header(
        ...
        "Answer a few money-first questions. The app will compare your current bill pathway with a strategy pathway and a subscription value test.",
        "💰",
    )
    ...
    with st.form("money_snapshot_form"):
        ...
        with c1:
            monthly_bill = st.number_input("Approximate monthly energy bill (A$)", min_value=0.0, max_value=3000.0, value=float(st.session_state.get("monthly_bill", 250.0)), step=10.0)
            subscription_fee = st.number_input("Possible advisor subscription (A$/month)", min_value=0.0, max_value=199.0, value=float(st.session_state.get("subscription_fee", 19.0)), step=1.0)
            dwelling_options = ["Small apartment / unit", "Townhouse / small house", "Detached house", "Large detached house"]
            dwelling_type = st.selectbox("What best describes the home?", dwelling_options, index=dwelling_options.index(st.session_state.get("dwelling_type", "Detached house")) if st.session_state.get("dwelling_type", "Detached house") in dwelling_options else 2)
        with c2:
            household_size = st.radio("People in the home", ["1", "2", "3-4", "5+"], horizontal=True, index=["1", "2", "3-4", "5+"].index(st.session_state.get("household_size", "3-4")) if st.session_state.get("household_size", "3-4") in ["1", "2", "3-4", "5+"] else 2)
    ...
    if submitted or "money_snapshot" in st.session_state:
        if submitted:
            st.session_state["monthly_bill"] = monthly_bill
            st.session_state["subscription_fee"] = subscription_fee
            st.session_state["household_size"] = household_size
            st.session_state["bill_problem"] = bill_problem
            st.session_state["dwelling_type"] = dwelling_type
            st.session_state["home_condition"] = home_condition
        ...
        snap = st.session_state["money_snapshot"]
        roi = roi_summary(snap["waste_low"], snap["waste_high"])
        st.markdown(
            f"""
            <div class='money-hero'>
                ...
                <p>If advisor access costs <strong>A${roi['monthly_fee']:.0f}/month</strong>, the annual access cost is <strong>{_currency(roi['annual_fee'])}</strong>. The gross saving opportunity is approximately <strong>{roi['multiple_low']:.1f}×–{roi['multiple_high']:.1f}×</strong> that annual fee.</p>
        ...
        with c3:
            value_bar("After subscription", f"{_currency(roi['net_low'])}–{_currency(roi['net_high'])}", "Indicative net opportunity after annual advisor fee.", "📈")
...
def profile_screen() -> None:
    ...
    with st.form("profile_form"):
        ...
        with c2:
            ...
            solar_status = st.radio("Does the home have solar panels?", ["Yes", "No", "Not sure"], horizontal=True)
            monthly_bill = st.number_input("Approximate monthly energy bill (A$)", min_value=0.0, max_value=3000.0, value=float(st.session_state.get("monthly_bill", 250.0)), step=10.0)
            subscription_fee = st.number_input("Advisor subscription value test (A$/month)", min_value=0.0, max_value=199.0, value=float(st.session_state.get("subscription_fee", 19.0)), step=1.0)
            condition_options = ["Newer / efficient", "Average / not sure", "Older or draughty"]
            home_condition = st.radio("Thermal feel of the home", condition_options, index=condition_options.index(st.session_state.get("home_condition", "Average / not sure")) if st.session_state.get("home_condition", "Average / not sure") in condition_options else 1)
...
def inspection_screen() -> None:
    episode_header(
        ...
        "Each check gives points and shows the likely saving logic. The final episode turns your answers into a subscription-value roadmap.",
        "⚡",
    )
    sidebar_status()
...
def plan_screen() -> None:
    ...
    roi = roi_summary(total_low, total_high)
    c1, c2, c3 = st.columns(3)
    with c1:
    ...
    with c3:
        value_bar("Net after advisor fee", f"{_currency(roi['net_low'])}–{_currency(roi['net_high'])}", f"After A${roi['monthly_fee']:.0f}/month annualised access cost.", "📈")
        ...
        with cols[idx]:
            ...
            st.markdown(
                ...
                <div class='card'>
                    ...
                    <p>{action['recommendation']}</p>
                    <p><strong>Indicative saving:</strong> {saving_text}</p>
                    <p><strong>Advisor value:</strong> this action is part of your annual {_currency(total_low)}–{_currency(total_high)} saving pathway.</p>
                </div>
                """,
                unsafe_allow_html=True,
            )

    st.markdown("### Episode 4 preview — what your subscription would keep doing")
    commercial_teaser_panel("Monthly advisor loop")
    st.markdown(
        """
        <div class='series-card'>
            <h4>Paid roadmap concept</h4>
            <p>The free result gives the first roadmap. A paid advisor mode would keep the user moving: monthly check-ins, proof-of-action photos, revised savings, next upgrade unlock, landlord letter for renters, and a running payback dashboard.</p>
    ...
    st.markdown("### Important limitation")
    st.warning("These results are indicative educational estimates only. A validated version should use local tariffs, climate zone, actual equipment power, hot-water system type, occupancy patterns, and pre/post evaluation data. Do not market the beta as a guaranteed bill-saving product.")

    if st.session_state.get("response_saved"):
        st.success(f"Anonymous response saved as {st.session_state.get('participant_id', 'Participant')}.")
    else:
        st.warning("Recommendations are being shown, but the anonymous response could not be saved. The tool still works, but check Streamlit Secrets, Google Sheet sharing permissions, and requirements.txt.")
        if st.session_state.get("save_error"):
            st.caption(f"Technical detail: {st.session_state.get('save_error')}")
        if st.button("Retry anonymous save", use_container_width=True):
            if save_anonymous_progress_if_needed(savings, ranked, result_text):

...
def render_hotspot(key: str) -> None:
    ...
    with st.expander(("✅ " if done else "🔎 ") + hotspot["label"] + f" — {hotspot['room']}", expanded=True if feedback else not done):
        ...
        if st.button("Check answer", key=f"check_{key}", disabled=answer is None or done):
            ...
            st.rerun()
        if feedback:
            css_class = "feedback-correct" if feedback["is_correct"] else "feedback-wrong"
            status = "Correct answer" if feedback["is_correct"] else "Not the best answer"
            correct_line = "" if feedback["is_correct"] else f"<div class='feedback-answer'><strong>Better choice:</strong> {feedback['correct']}</div>"
            st.markdown(
                f"""
                <div class='{css_class}'>
                    <strong>{status}</strong>
                    <div class='feedback-answer'><strong>Your selection:</strong> {feedback['selected']}</div>
                    {correct_line}
                    <div class='feedback-answer'>{feedback['message']}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )

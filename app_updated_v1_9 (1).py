...
def plan_screen() -> None:
    ...
    certificate_name = st.session_state.get("participant_id", "Anonymous Participant")
    ...
    st.markdown("### Anonymous response submission")
    st.info(
        "You may submit your completed responses for tool evaluation and improvement. "
        "No name, email address, or contact detail will be collected or saved. "
        "Your record will be stored using an anonymous participant ID."
    )
    consent_given = st.checkbox(
        "I agree that my anonymous responses may be stored for tool evaluation and improvement.",
        key="anonymous_consent",
    )

    if st.session_state.get("response_saved"):
        st.success(f"Your anonymous response has already been saved as {st.session_state.get('participant_id', 'Participant')}.")
    elif consent_given:
        if st.button("Submit anonymous results", type="primary", use_container_width=True):
            try:
                conn = st.connection("gsheets", type=GSheetsConnection)
        ...
        st.caption("Tick the consent box only if you want to submit your anonymous results.")

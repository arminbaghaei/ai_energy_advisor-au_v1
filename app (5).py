...
def money_screen() -> None:
    ...
    with st.form("money_snapshot_form"):
        ...
        render_visual_options("Home-type picture guide", DWELLING_VISUAL_CARDS)
        render_visual_options("Thermal-feel picture guide", THERMAL_VISUAL_CARDS)
...
.value-number, .money-number, .payoff-strip strong {
    background:#FEF3C7;
    border:1px solid #F59E0B;
    border-radius:.7rem;
    padding:.12rem .45rem;
...
.visual-choice-icon {font-size:2rem; line-height:1; margin-bottom:.35rem;}
.visual-choice-title {font-weight:900; color:#0F172A; font-size:.82rem; line-height:1.2;}
.visual-choice-note {color:#64748B; font-size:.70rem; line-height:1.25; margin-top:.18rem;}
.visual-strip {
    padding:.85rem;
    border-radius:18px;
    background:#F8FAFC;
...
.visual-strip-title {font-weight:900; color:#0F172A; margin-bottom:.45rem;}
@media (max-width: 900px) {.visual-choice-grid {grid-template-columns:repeat(2,minmax(0,1fr));}}
@media (max-width: 520px) {.visual-choice-grid {grid-template-columns:1fr;}}
</style>
""", unsafe_allow_html=True)
...
def render_visual_options(title: str, cards: list[tuple[str, str, str]]) -> None:
...
HVAC_VISUAL_CARDS = [
    ("❄️", "Split-system AC", "Heating and cooling"),
    ("🔥", "Gas heater", "Winter comfort"),
    ("♨️", "Portable electric", "High running risk"),
    ("🌬️", "Ducted system", "Whole-home airflow"),
]
...
CHALLENGE_VISUAL_CARDS = [
    ...
    ("🌡️", "Thermostat", "Set-point discipline"),
]

HOTSPOT_OPTION_VISUALS = {
    "thermostat": [
        ("🥶", "18°C cooling", "High cooling load"),
        ("✅", "25–27°C cooling", "Better summer setting"),
        ("🥵", "26°C heating", "High winter load"),
    ],
    "leds": [
    ...
    "curtains": [
        ...
        ("☀️", "Summer shading", "Cuts cooling load"),
    ],
    "standby": [
        ("🔌", "Wall switch", "Stop standby load"),
        ("🧠", "Smart board", "Manage multiple devices"),
        ("📺", "Always-on devices", "Hidden money leak"),
    ],
    "shower": [
    ...
    "draught": [
        ...
        ("🪟", "Weather strip", "Window/door gaps"),
        ("🌬️", "Draught", "Paid air escaping"),
    ],
    "insulation": [
        ("🏠", "Ceiling insulation", "Long-term comfort"),
...
def render_hotspot_option_visuals(key: str) -> None:
    ...
    if cards:
        render_visual_options("Visual guide", cards)
...
def profile_screen() -> None:
    ...
    with st.form("profile_form"):
        ...
        with c2:
            ...
            condition_options = ["Newer / efficient", "Average / not sure", "Older or draughty"]
            home_condition = st.radio("Thermal feel of the home", condition_options, index=condition_options.index(st.session_state.get("home_condition", "Average / not sure")) if st.session_state.get("home_condition", "Average / not sure") in condition_options else 1)
        render_visual_options("Home and system picture guide", DWELLING_VISUAL_CARDS + HVAC_VISUAL_CARDS[:2])
        render_visual_options("Solar and thermal clues", SOLAR_VISUAL_CARDS + THERMAL_VISUAL_CARDS[:1])
        ...
        render_visual_options("Assumption picture guide", CHALLENGE_VISUAL_CARDS)
...
# Render the account/project sidebar on every page.

...
.certificate-card::after {
    content: "EnergyWise Recognition";
    position: absolute;
    top: 24px;
    right: -44px;
...
.certificate-title {font-size: 2rem; font-weight: 900; color: #0F172A; margin: 0.35rem 0 0.2rem 0; letter-spacing: -0.02em;}
.certificate-subtitle {font-size: 0.95rem; color: #475569; margin-bottom: 1.2rem;}
.certificate-name {font-size: 1.75rem; font-weight: 900; color: #0F766E; margin: 0.75rem auto; padding-bottom:0.35rem; border-bottom: 2px solid #99F6E4; max-width: 560px;}
.certificate-small {font-size: 0.94rem; color: #334155; line-height: 1.55;}
.certificate-meta {display:flex; justify-content:center; gap:0.75rem; flex-wrap:wrap; margin-top:1rem;}
.certificate-pill {padding:0.45rem 0.75rem; border-radius:999px; background:#ECFDF5; border:1px solid #A7F3D0; color:#065F46; font-weight:700; font-size:0.84rem;}
.certificate-disclaimer {font-size:0.78rem; color:#64748B; margin-top:1rem;}
...
def generate_certificate_png(name: str, score: int, result_label: str) -> bytes:
    name = name.strip() or "EnergyWise Participant"
    today = date.today().strftime("%d %B %Y")
    width, height = 1600, 1050
    img = Image.new("RGB", (width, height), "#EEF2F7")
    draw = ImageDraw.Draw(img)

    # Outer frame and main certificate surface
    draw.rounded_rectangle((70, 70, width - 70, height - 70), radius=42, fill="#FFFFFF", outline="#0F766E", width=8)
    draw.rounded_rectangle((108, 108, width - 108, height - 108), radius=28, outline="#99F6E4", width=3)
    draw.rounded_rectangle((132, 132, width - 132, height - 132), radius=20, outline="#E2E8F0", width=2)

    # Decorative accents
    draw.rectangle((70, 70, width - 70, 210), fill="#0F766E")
    draw.polygon([(70, 210), (250, 210), (160, 295)], fill="#0EA5E9")
    draw.polygon([(width - 70, 210), (width - 250, 210), (width - 160, 295)], fill="#0EA5E9")
    draw.ellipse((122, 260, 214, 352), outline="#0F766E", width=4)
    draw.ellipse((width - 214, 260, width - 122, 352), outline="#0F766E", width=4)

    # Logo on a white plaque for visibility
    plaque_w, plaque_h = 300, 112
    plaque_x0 = int((width - plaque_w) / 2)
    draw.rounded_rectangle((plaque_x0, 98, plaque_x0 + plaque_w, 98 + plaque_h), radius=18, fill="#FFFFFF", outline="#CCFBF1", width=2)
    if LOGO_PATH.exists():
        try:
            logo = Image.open(LOGO_PATH).convert("RGBA")
            logo.thumbnail((230, 78))
            logo_x = int((width - logo.width) / 2)
            logo_y = 115 + int((78 - logo.height) / 2)
            img.paste(logo, (logo_x, logo_y), logo)
        except Exception:
            pass
    else:
        _draw_centered(draw, "Tech Innovation Experts", 132, _load_font(26, True), "#0F172A", width)

    y = 270
    y = _draw_centered(draw, "CERTIFICATE OF COMPLETION", y, _load_font(54, True), "#0F172A", width)
    y = _draw_centered(draw, "EnergyWise Homes Australia", y + 6, _load_font(31, True), "#0F766E", width)
    y = _draw_centered(draw, "recognises", y + 24, _load_font(27), "#475569", width)

    # Participant name with underline
    name_font = _load_font(58, True)
    name_lines = textwrap.wrap(name, width=30) or [name]
    for line in name_lines[:2]:
        y = _draw_centered(draw, line, y + 6, name_font, "#0F172A", width)
    draw.line((420, y + 2, width - 420, y + 2), fill="#99F6E4", width=4)
    y += 32

    body = "for completing the home-energy check-up and building a practical action plan for identifying common household energy-wasting features."
    y = _draw_wrapped_centered(draw, body, y, _load_font(28), "#334155", width, wrap=78, line_gap=10)
    y += 24
    ...
    badge_widths = [draw.textbbox((0, 0), b, font=_load_font(24, True))[2] + 58 for b in badges]
    total_w = sum(badge_widths) + 28 * (len(badges) - 1)
    x = int((width - total_w) / 2)
    for b, bw in zip(badges, badge_widths):
        draw.rounded_rectangle((x, badge_y, x + bw, badge_y + badge_h), radius=28, fill="#ECFDF5", outline="#A7F3D0", width=2)
        bbox = draw.textbbox((0, 0), b, font=_load_font(24, True))
        draw.text((x + (bw - (bbox[2] - bbox[0])) / 2, badge_y + 16), b, font=_load_font(24, True), fill="#065F46")
        x += bw + 28

    # Recognition seal
    seal_cx, seal_cy, seal_r = 800, 820, 70
    draw.ellipse((seal_cx - seal_r, seal_cy - seal_r, seal_cx + seal_r, seal_cy + seal_r), fill="#0F766E", outline="#0EA5E9", width=6)
    _draw_centered(draw, "EW", seal_cy - 38, _load_font(38, True), "#FFFFFF", width)
    _draw_centered(draw, "RECOGNISED", seal_cy + 2, _load_font(16, True), "#CCFBF1", width)

    # Footer
    footer_y = 930
    _draw_centered(draw, "Tech Innovation Experts", footer_y, _load_font(24, True), "#0F172A", width)
    _draw_centered(draw, "Providing technology-driven services across Oceania | tinx@gmail.com", footer_y + 36, _load_font(21), "#475569", width)
    _draw_centered(draw, "Recognition certificate only. Not a certified energy assessment or NCC/NatHERS rating.", height - 58, _load_font(18), "#64748B", width)
...
def share_links_html(name: str) -> str:
    share_text = "I completed the EnergyWise Homes Australia home-energy check-up and received my Energy Action Plan."
    if name.strip():
        share_text = f"{name.strip()} completed the EnergyWise Homes Australia home-energy check-up and received an Energy Action Plan."
...
def plan_screen() -> None:
    ...
    participant_name = st.session_state.get("participant_name", "").strip() or "EnergyWise Participant"
    ...
    st.markdown(
        ...
        <div class='certificate-card'>
            ...
            <div class='certificate-subtitle'>EnergyWise Homes Australia</div>
    ...
    st.download_button(
        ...
        certificate_png,
        "energywise_completion_certificate.png",
        "image/png",
        use_container_width=True,
    )
    st.caption("For LinkedIn, Facebook, or X: download the certificate image first, then attach it to your post. The buttons below open a draft share link.")
    st.markdown(share_links_html(participant_name), unsafe_allow_html=True)

...
def auth_panel(compact: bool = False) -> None:
    ...
    This uses only email and password for authentication. The app should not ask for
    full name, phone number, exact address, or utility account details.
    """
    if is_logged_in():
    ...
    if compact:
        st.info("Create an account to save this roadmap and track progress over time.")
        ...
        if submitted:
            ...
            if supabase is None:
                ...
                try:
                    ...
                    st.error("Login failed. Check the email and password.")
                    st.caption(str(exc))
    ...
    with signup_tab:
        with st.form("signup_form"):
            ...
            household_nickname = st.text_input("Household nickname", value="My Home", key="signup_home")
            ...
            if supabase is None:
                ...
                try:
                    ...
                    if user:
                        st.session_state["auth_user_id"] = user.id
                        st.session_state["auth_email"] = user.email
                        save_profile_to_supabase(household_nickname=household_nickname)
...
def load_user_dashboard_data() -> dict:
    ...
    try:
        scans = supabase.table("energy_scans").select("*").eq("user_id", uid).order("created_at", desc=True).limit(10).execute().data or []
        recommendations = supabase.table("recommendations").select("*").eq("user_id", uid).order("created_at", desc=True).limit(50).execute().data or []
        checkins = supabase.table("monthly_checkins").select("*").eq("user_id", uid).order("checkin_month", desc=False).limit(24).execute().data or []
...
def dashboard_screen() -> None:
    ...
    if not scans:
        st.info("No saved scan yet. Complete the Energy Money Leak Scan and unlock recommendations to save your first roadmap.")
        if st.button("Start my first scan", type="primary", use_container_width=True):
            next_stage("money")
        return
...
# Override welcome_screen to include account access without forcing login.
_original_welcome_screen_for_accounts = welcome_screen

def welcome_screen() -> None:
    _original_welcome_screen_for_accounts()
    st.markdown("### Returning user?")
    if is_logged_in():
        if st.button("Open my saved energy dashboard", use_container_width=True):
            next_stage("dashboard")
    else:
        st.info("You can use the tool as a guest, or create an account later to save and track your progress.")
        if st.button("Create or log in to an account", use_container_width=True):
            next_stage("account")

...
def sidebar_account_card() -> None:
    ...
    if is_logged_in():
        ...
        project = st.session_state.get("household_nickname", "Home 1")
...
def dashboard_screen() -> None:
    ...
    st.markdown("### My household project")
    active_project_name = st.session_state.get("household_nickname", "Home 1")
    st.markdown(
        f"""
        <div class='project-box'>
            <strong>Active project:</strong> {active_project_name}<br>
            This version tracks one household project. A future version can allow multiple homes, such as Home 1, Home 2, rental property, or holiday home.
        </div>
        <div class='locked-project'>
            <strong>Add another home:</strong> locked in this version. This can become a paid/advanced feature later.
        </div>
        """,
        unsafe_allow_html=True,
    )

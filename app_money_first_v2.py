...
def inspection_screen() -> None:
    ...
    st.markdown("### Annual energy-cost pathway")
    st.caption("The shaded band is a benchmark-style standard operating range for an efficient household routine. The current input line shows the user's estimated cost pathway. The strategy line shows how costs can move toward the standard range after behaviour and low-cost actions. This is not an NCC/NatHERS compliance result.")
...
def plan_screen() -> None:
    ...
    st.markdown("### Please submit your anonymous response")
    st.markdown(
        """
        <div style="padding:1.15rem; border-radius:18px; background:#ECFDF5; border:1px solid #A7F3D0; margin-bottom:0.75rem;">
            <h4 style="margin-top:0; color:#065F46;">Help improve this tool</h4>
            <p style="margin-bottom:0.45rem; color:#064E3B;">
                Your completed responses are valuable for evaluating and improving <strong>The Home-energy check-up (Australia)</strong>.
                Please submit your responses if you are comfortable contributing anonymous evaluation data.
            </p>
            <p style="margin-bottom:0; color:#334155; font-size:0.93rem;">
                No name, email address, phone number, or contact detail will be collected or saved. Your record will be stored only with an anonymous participant ID, such as Participant 1 or Participant 2.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    consent_given = st.checkbox(
        "I agree to submit my anonymous responses for tool evaluation and improvement.",
        key="anonymous_consent",
    )

    if st.session_state.get("response_saved"):
        st.success(f"Thank you — your anonymous response has been saved as {st.session_state.get('participant_id', 'Participant')}.")
    elif consent_given:
        if st.button("Submit my anonymous responses", type="primary", use_container_width=True):
            try:
                conn = st.connection("gsheets", type=GSheetsConnection)
                ...
                st.success(f"Thank you. Your anonymous results have been saved as {participant_id}.")
                st.rerun()
            except Exception as exc:
                st.error(
                    "The app could not save your response to Google Sheets. "
                    "Check Streamlit Secrets, Google Sheet sharing permissions, and requirements.txt."
                )
                st.caption(f"Technical detail: {exc}")
    else:
        st.warning("Please tick the consent box and submit your anonymous responses if you are willing to help us improve this tool.")
